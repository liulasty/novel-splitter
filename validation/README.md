...
## 核心职责
- **定义校验规范**：提供统一的校验结果对象 `ValidationResult`，标准化校验输出。
- **提供基础校验实现**：内置基于长度等基础属性的校验逻辑（`LengthLimitStrategy`），保证生成的片段满足大模型及向量数据库的要求。
...
## 扩展点
- **扩展点 1**：可实现自定义的 `DialogueStrategy` 或 `LengthLimitStrategy`，并集成到 `SemanticSegmentBuilder`，调整组装逻辑。
