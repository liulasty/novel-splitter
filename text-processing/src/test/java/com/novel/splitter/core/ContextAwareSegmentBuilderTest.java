...
class ContextAwareSegmentBuilderTest {
    ...
    @Test
    void testSemanticMergeByTokenOverlap() {
        List<RawParagraph> inputs = new ArrayList<>();
        inputs.add(createPara(1, "勇者 在 森林 与 巨龙 对峙"));
        inputs.add(createPara(2, "勇者 在 森林 继续 对峙 巨龙"));
    ...
    @Test
    void testSemanticCutByTokenOverlap() {
        List<RawParagraph> inputs = new ArrayList<>();
        inputs.add(createPara(1, "舰队 在 近地轨道 进行 对接 操作"));
        inputs.add(createPara(2, "宫廷 舞会 上 乐师 正在 演奏 圆舞曲"));
