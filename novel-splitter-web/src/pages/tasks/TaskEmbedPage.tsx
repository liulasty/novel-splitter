...
export default function TaskEmbedPage() {
  const [novelId, setNovelId] = useState('');
  const [version, setVersion] = useState('v1');
  const [taskId, setTaskId] = useState<string | null>(null);

  const submit = useMutation({
    mutationFn: () => novelApi.embedNovel(novelId.trim(), version.trim() || 'v1'),
    onSuccess: (r) => {
      setTaskId(r.taskId);
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Embed（向量化）</h1>
        <p className="text-sm text-gray-500 mt-1">POST /api/novels/&#123;novelId&#125;/embed?version=</p>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 space-y-4 shadow-sm">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-xl bg-violet-600 text-white px-4 py-2 text-sm font-medium disabled:opacity-50"
        >
